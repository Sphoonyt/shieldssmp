package com.shieldssmp.listeners;

import com.shieldssmp.ShieldsSMP;
import com.shieldssmp.classes.PlayerClass;
import com.shieldssmp.classes.impl.PhantomClass;
import com.shieldssmp.classes.impl.TerroristClass;
import com.shieldssmp.items.SpecialItems;
import com.shieldssmp.systems.ClassManager;
import com.shieldssmp.systems.LifeSystem;
import org.bukkit.entity.Player;
import java.util.UUID;
import org.bukkit.event.EventHandler;
import org.bukkit.event.EventPriority;
import org.bukkit.event.Listener;
import org.bukkit.event.block.BlockBreakEvent;
import org.bukkit.event.entity.*;
import org.bukkit.event.player.*;
import org.bukkit.inventory.ItemStack;

public class GlobalListener implements Listener {

    private final ShieldsSMP plugin;
    private final ClassManager cm;
    private final LifeSystem lifeSystem;
    private final SpecialItems specialItems;

    public GlobalListener(ShieldsSMP plugin) {
        this.plugin       = plugin;
        this.cm           = plugin.getClassManager();
        this.lifeSystem   = plugin.getLifeSystem();
        this.specialItems = plugin.getSpecialItems();
    }

    // ── Player join / quit ────────────────────────────────────────────────────

    @EventHandler
    public void onJoin(PlayerJoinEvent event) {
        cm.loadPlayer(event.getPlayer());
        // Shield is given/verified by ShieldListener.onFirstJoin which fires after this
        // Sync shield durability to saved lives count (in case they changed offline)
        plugin.getServer().getScheduler().runTaskLater(plugin,
                () -> plugin.getLifeSystem().syncOnJoin(event.getPlayer()), 10L);
        lifeSystem.sendLivesBar(event.getPlayer(), lifeSystem.getLives(event.getPlayer().getUniqueId()));
    }

    @EventHandler
    public void onQuit(PlayerQuitEvent event) {
        cm.unloadPlayer(event.getPlayer());
    }

    // ── Death ─────────────────────────────────────────────────────────────────

    @EventHandler(priority = EventPriority.MONITOR)
    public void onDeath(PlayerDeathEvent event) {
        Player player = event.getEntity();
        lifeSystem.handleDeath(player);

        // If player already has 0 lives, skip all death processing (shield already broken)
        var preCheckData = cm.getPlayerData(player.getUniqueId());
        if (preCheckData.getLives() <= 0) {
            // Still call class death hook for cleanup
            PlayerClass zeroCls = cm.getPlayerClass(player.getUniqueId());
            if (zeroCls != null) zeroCls.onDeath(player);
            return;
        }

        // Shielded Totem Soul Guard – skip life deduction if totem saved them
        if (plugin.getMythicalItemListener().consumeTotemProtection(player.getUniqueId())) {
            // Totem handled – just call class death hook, skip life loss
            PlayerClass totemCls = cm.getPlayerClass(player.getUniqueId());
            if (totemCls != null) totemCls.onDeath(player);
            return;
        }

        // If player was Level 2, drop their Upgrade Core and revert to Level 1
        var data = cm.getPlayerData(player.getUniqueId());
        if (data.getLevel() >= 2) {
            data.setLevel(1);
            data.save(plugin);
            // Drop upgrade core at death location
            player.getWorld().dropItemNaturally(player.getLocation(), specialItems.buildUpgradeCore());
            player.sendMessage(net.kyori.adventure.text.Component.text(
                    "[ShieldsSMP] Your Upgrade Core dropped on death!", net.kyori.adventure.text.format.NamedTextColor.YELLOW));
        }

        PlayerClass cls = cm.getPlayerClass(player.getUniqueId());
        if (cls != null) cls.onDeath(player);
    }

    // ── Kill hook ─────────────────────────────────────────────────────────────

    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onEntityDeath(EntityDeathEvent event) {
        if (event.getEntity().getKiller() == null) return;
        Player killer = event.getEntity().getKiller();
        PlayerClass cls = cm.getPlayerClass(killer.getUniqueId());
        if (cls != null && cm.isHoldingShield(killer)) cls.onKill(killer, event.getEntity());
        // Explicit Mimic kill recording (covers all kill types reliably) – also requires shield
        if (cls instanceof com.shieldssmp.classes.impl.LarpClass larp && cm.isHoldingShield(killer)) {
            larp.recordKill(killer, event.getEntityType());
        }
    }

    // ── Damage hooks ──────────────────────────────────────────────────────────

    @EventHandler(priority = EventPriority.HIGH, ignoreCancelled = true)
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        // Nullifier Axe
        if (event.getDamager() instanceof Player attacker) {
            ItemStack weapon = attacker.getInventory().getItemInMainHand();
            if (specialItems.isNullifierAxe(weapon)) {
                if (!cm.isOnNullifierCooldown(attacker.getUniqueId())) {
                    if (event.getEntity() instanceof Player victim) {
                        cm.disableAbilities(victim, 60_000L);
                        cm.putNullifierCooldown(attacker.getUniqueId());
                        attacker.setCooldown(org.bukkit.Material.NETHERITE_AXE, 3 * 60 * 20); // 3min visual
                        attacker.sendActionBar(net.kyori.adventure.text.Component.text(
                                "☠ Nullifier hit " + victim.getName() + "!", net.kyori.adventure.text.format.NamedTextColor.DARK_RED));
                    }
                }
            }

            // Attacker class damage hook (blocked at 0 lives or without shield in hand)
            if (event.getEntity() instanceof org.bukkit.entity.LivingEntity le) {
                PlayerClass cls = cm.getPlayerClass(attacker.getUniqueId());
                boolean attackerHasShield = cm.isHoldingShield(attacker);
                if (cls != null && !cm.abilitiesDisabled(attacker.getUniqueId())
                        && plugin.getLifeSystem().hasLives(attacker.getUniqueId())
                        && attackerHasShield) {
                    cls.onDealDamage(attacker, le, event.getDamage());
                }
                // Blink: True Invis – 1.5x outgoing damage (requires shield in hand)
                if (cls instanceof com.shieldssmp.classes.impl.BlinkClass bc
                        && bc.isTrueInvisActive(attacker.getUniqueId())
                        && attackerHasShield) {
                    event.setDamage(event.getDamage() * 1.5);
                }
            }
        }

        // Victim class damage hook
        if (event.getEntity() instanceof Player victim) {
            PlayerClass cls = cm.getPlayerClass(victim.getUniqueId());
            boolean victimHasShield = cm.isHoldingShield(victim);
            if (cls != null && !cm.abilitiesDisabled(victim.getUniqueId())
                    && plugin.getLifeSystem().hasLives(victim.getUniqueId())
                    && victimHasShield) {
                cls.onTakeDamage(victim, event.getDamager(), event.getDamage());
            }
            // Blood: Crimson Frenzy – take 30% more damage (requires shield in hand)
            if (cls instanceof com.shieldssmp.classes.impl.BloodClass bc
                    && bc.isFrenzied(victim.getUniqueId())
                    && victimHasShield) {
                event.setDamage(event.getDamage() * 1.3);
            }
            // Blink: True Invis – take 2x damage (requires shield in hand)
            if (cls instanceof com.shieldssmp.classes.impl.BlinkClass blink
                    && blink.isTrueInvisActive(victim.getUniqueId())
                    && victimHasShield) {
                event.setDamage(event.getDamage() * 2.0);
            }
            // Blink: Phase Shift passive – 5% chance to dodge (requires shield in hand)
            if (cls instanceof com.shieldssmp.classes.impl.BlinkClass blinkDodge
                    && victimHasShield
                    && blinkDodge.rollDodge()) {
                event.setCancelled(true);
                victim.sendActionBar(net.kyori.adventure.text.Component.text(
                        "⚡ Phase Shift: dodged!", net.kyori.adventure.text.format.NamedTextColor.DARK_AQUA,
                        net.kyori.adventure.text.format.TextDecoration.BOLD));
            }

            // Combustion: only block explosions from their OWN abilities (requires shield in hand)
            if (cls instanceof TerroristClass && victimHasShield) {
                if ((event.getCause() == EntityDamageEvent.DamageCause.ENTITY_EXPLOSION
                        || event.getCause() == EntityDamageEvent.DamageCause.BLOCK_EXPLOSION)
                        && isCombustionOwnExplosion(event)) {
                    event.setCancelled(true);
                }
                // TNT minecarts always deal damage (not blocked)
            }
        }
    }

    @EventHandler(priority = EventPriority.HIGH, ignoreCancelled = true)
    public void onExplosion(EntityDamageEvent event) {
        if (!(event.getEntity() instanceof Player p)) return;
        boolean pHasShield = cm.isHoldingShield(p);

        // Super Supernova fall damage cancel (requires shield in hand)
        if (event.getCause() == EntityDamageEvent.DamageCause.FALL) {
            PlayerClass cls2 = cm.getPlayerClass(p.getUniqueId());
            if (cls2 instanceof com.shieldssmp.classes.impl.SuperClass sc
                    && pHasShield && sc.cancelFallDamage(p.getUniqueId())) {
                event.setCancelled(true); return;
            }
        }

        if (event.getCause() != EntityDamageEvent.DamageCause.ENTITY_EXPLOSION
                && event.getCause() != EntityDamageEvent.DamageCause.BLOCK_EXPLOSION) return;

        PlayerClass cls = cm.getPlayerClass(p.getUniqueId());
        if (cls instanceof TerroristClass && pHasShield && isCombustionOwnExplosion(event)) {
            event.setCancelled(true);
        }
        // TNT minecarts and other explosions still damage Combustion players
    }

    /**
     * Returns true if the explosion was caused by a TNT entity that was
     * spawned by the Combustion class (has carpet_bomb_owner or nuke_owner metadata).
     * Used to let only OWN ability explosions be blocked.
     */
    private boolean isCombustionOwnExplosion(EntityDamageEvent event) {
        if (!(event instanceof org.bukkit.event.entity.EntityDamageByEntityEvent byEntity)) return false;
        org.bukkit.entity.Entity damager = byEntity.getDamager();
        // Primed TNT with carpet_bomb_owner or nuke_owner metadata = own ability
        return damager.hasMetadata("carpet_bomb_owner") || damager.hasMetadata("nuke_owner");
    }

    // ── Block break – reveal Phantom ──────────────────────────────────────────

    @EventHandler(ignoreCancelled = true)
    public void onBreakBlock(BlockBreakEvent event) {
        Player player = event.getPlayer();
        PlayerClass cls = cm.getPlayerClass(player.getUniqueId());
        if (cls != null && cm.isHoldingShield(player)) cls.onBreakBlock(player);
    }

    // ── Item consumption (Life, Golden Apple hooks) ────────────────────────────

    @EventHandler
    public void onConsume(PlayerItemConsumeEvent event) {
        Player player = event.getPlayer();
        ItemStack item = event.getItem();

        // Life Essence right-click pickup
        if (specialItems.isLifeEssence(item)) {
            event.setCancelled(true);
            if (lifeSystem.tryGiveLife(player, item)) {
                player.getInventory().getItemInMainHand().subtract(1);
            }
            return;
        }

        // Upgrade Core
        if (specialItems.isUpgradeCore(item)) {
            event.setCancelled(true);
            if (lifeSystem.tryUpgrade(player, item)) {
                player.getInventory().getItemInMainHand().subtract(1);
            }
            return;
        }

        // Class passive consumption hook (e.g. Life's enhanced golden apple) – requires shield
        PlayerClass cls = cm.getPlayerClass(player.getUniqueId());
        if (cls != null && cm.isHoldingShield(player)) cls.onConsumeItem(player, item);
    }

    // ── Life Essence / Upgrade Core right-click (held item, not edible) ───────

    @EventHandler
    public void onInteract(PlayerInteractEvent event) {
        Player player = event.getPlayer();
        ItemStack item = player.getInventory().getItemInMainHand();
        if (item == null) return;

        if (specialItems.isLifeEssence(item)) {
            event.setCancelled(true);
            if (lifeSystem.tryGiveLife(player, item)) {
                item.subtract(1);
            }
            return;
        }

        if (specialItems.isUpgradeCore(item)) {
            event.setCancelled(true);
            if (lifeSystem.tryUpgrade(player, item)) {
                item.subtract(1);
            }
            return;
        }

        // Revive Book is handled entirely in ShieldListener.onReviveBook
        // (no action needed here)
    }

    // ── Phantom: cancel attack-based reveal in spectral form ──────────────────

    // Block boss minions from targeting their owner
    @EventHandler(priority = EventPriority.HIGH)
    public void onEntityTarget(EntityTargetLivingEntityEvent event) {
        if (!(event.getTarget() instanceof Player owner)) return;
        UUID ownerId = owner.getUniqueId();
        // Check if the attacker is one of the owner's minions (already-summoned minions
        // never turn on their owner regardless of current hand item)
        com.shieldssmp.classes.PlayerClass cls = cm.getPlayerClass(ownerId);
        if (cls instanceof com.shieldssmp.classes.impl.BossClass bc) {
            if (bc.isMinionOf(event.getEntity().getUniqueId(), ownerId)) {
                event.setCancelled(true);
            }
        }
        // Sovereign passive (blocks ALL mob aggro) – requires shield in main hand
        if (cls instanceof com.shieldssmp.classes.impl.BossClass && cm.isHoldingShield(owner)) {
            if (!(event.getEntity() instanceof org.bukkit.entity.Player)) {
                event.setCancelled(true);
            }
        }
    }

    @EventHandler(priority = EventPriority.HIGH)
    public void onPhantomSpectralAttack(EntityDamageByEntityEvent event) {
        if (!(event.getDamager() instanceof Player attacker)) return;
        PlayerClass cls = cm.getPlayerClass(attacker.getUniqueId());
        if (cls instanceof PhantomClass phantom) {
            if (phantom.isSpectral(attacker.getUniqueId())) {
                event.setCancelled(true); // Cannot attack in spectral realm
            }
        }
    }
}
